...
function toonToernooiGroepen(spelers, kalender = []) {
  ...
  const wedstrijdenContainer = document.getElementById('wedstrijden-visualisatie');
  ...
  if (!deelnemersContainer || !groepenContainer || !feedbackElement || !exportKnop) {
    return;
  }
  ...
  const setFeedback = (bericht, type = 'info') => {
    const basisKlassen = ['text-sm', 'rounded-lg', 'p-4', 'border', 'border-dashed'];
    feedbackElement.className = basisKlassen.join(' ');

    if (type === 'success') {
      feedbackElement.classList.add('text-green-600', 'bg-green-50', 'border-green-200');
    } else if (type === 'error') {
      feedbackElement.classList.add('text-red-600', 'bg-red-50', 'border-red-200');
    } else {
      feedbackElement.classList.add('text-gray-600', 'bg-gray-50', 'border-gray-300');
    }
  ...
  const renderGroepen = (groepen) => {
    ...
    groepen.forEach(groep => {
      ...
      kaart.className = 'rounded-lg border border-gray-200 bg-white p-4 shadow-sm';
      ...
      if (groep.spelers.length === 0) {
        ...
        groep.spelers.forEach(speler => {
          ...
          item.className = 'flex items-center justify-between text-sm text-gray-700';
  ...
  const renderWedstrijden = (schemaPerGroep) => {
    ...
    const heeftWedstrijden = Array.isArray(schemaPerGroep) && schemaPerGroep.some(groep =>
      Array.isArray(groep.rondes) && groep.rondes.some(ronde => Array.isArray(ronde.wedstrijden) && ronde.wedstrijden.length > 0)
    );

    if (!heeftWedstrijden) {
      const melding = document.createElement('p');
      melding.className = 'text-sm text-gray-500 italic';
      melding.textContent = 'De wedstrijden verschijnen hier zodra er voldoende spelers zijn geselecteerd.';
    ...
    schemaPerGroep.forEach(groepSchema => {
      const kaart = document.createElement('div');
      kaart.className = 'space-y-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm';
      ...
      const rondesCount = Array.isArray(groepSchema.rondes) ? groepSchema.rondes.length : 0;
      ...
      if (!groepSchema.rondes || groepSchema.rondes.length === 0) {
        ...
        leeg.textContent = 'Niet genoeg spelers in deze groep voor wedstrijden.';
        kaart.appendChild(leeg);
      } else {
        ...
        groepSchema.rondes.forEach(ronde => {
          ...
          rondeKaart.className = 'space-y-3 rounded-lg border border-slate-200 bg-slate-50 p-3 shadow-sm';

          const rondeTitel = document.createElement('h4');
          rondeTitel.className = 'text-sm font-semibold uppercase tracking-wide text-slate-700';
          ...
          ronde.wedstrijden.forEach(wedstrijd => {
            const item = document.createElement('li');
            item.className = 'flex items-center justify-between gap-3 rounded-md border border-white bg-white px-3 py-2 shadow-sm';
            ...
            tag.className = 'text-xs font-semibold uppercase tracking-wide text-slate-400';
  ...
  const checkboxElementen = () => Array.from(deelnemersContainer.querySelectorAll('input[type="checkbox"]'));

  const updateGroepen = () => {
    const geselecteerdeSpelers = checkboxElementen()
      .filter(checkbox => checkbox.checked)
      .map(checkbox => gesorteerdeSpelers[parseInt(checkbox.dataset.index || '0', 10)])
      .filter(Boolean);

    resetCsvFeedback();

    if (geselecteerdeSpelers.length === 0) {
      const legeGroepen = verdeelSpelersInGroepen([]);
      renderGroepen(legeGroepen);
      renderWedstrijden([]);
      setFeedback('Selecteer minimaal vier spelers om de groepen automatisch te vormen.', 'info');
    ...
    if (geselecteerdeSpelers.length < 4) {
      ...
      renderGroepen(legeGroepen);
      renderWedstrijden([]);
      setFeedback('Er zijn minstens vier aanwezige spelers nodig om de groepen te vormen.', 'error');
      exportKnop.disabled = true;
      laatsteWedstrijden = [];
      return;
    }

    const groepen = verdeelSpelersInGroepen(geselecteerdeSpelers);
    renderGroepen(groepen);

    setFeedback(`Er zijn ${geselecteerdeSpelers.length} spelers geselecteerd. De groepen zijn opgebouwd volgens het klassement.`, 'success');
  ...
  gesorteerdeSpelers.forEach((speler, index) => {
    ...
    label.className = 'flex items-center justify-between gap-3 rounded-lg border border-gray-200 bg-white px-3 py-2 shadow-sm hover:border-[#76A936] transition-colors';
    ...
    checkbox.className = 'h-4 w-4 text-[#76A936] border-gray-300 rounded focus:ring-[#76A936]';
  ...
  // Initial rendering
  renderGroepen(verdeelSpelersInGroepen([]));
  renderWedstrijden([]);
  setFeedback('Selecteer minimaal vier spelers om de groepen automatisch te vormen.', 'info');
  resetCsvFeedback();
}
...
function vindToernooiDatum(kalender = []) {
  ...
  const toernooiItems = metGeldigeDatum.filter(entry => (entry.type || '').toLowerCase() === 'toernooi');
  const kandidaten = (toernooiItems.length > 0 ? toernooiItems : metGeldigeDatum)
    .slice()
...
function verdeelSpelersInGroepen(spelers) {
  const groepNamen = ['Groep A', 'Groep B', 'Groep C', 'Groep D'];
  const groepen = groepNamen.map(naam => ({ naam, spelers: [] }));
  ...
  const extraSpelers = spelers.length % aantalGroepen;

  let startIndex = 0;
  groepen.forEach((groep, index) => {
    const doelGrootte = basisGrootte + (index < extraSpelers ? 1 : 0);
    groep.spelers = doelGrootte > 0 ? spelers.slice(startIndex, startIndex + doelGrootte) : [];
