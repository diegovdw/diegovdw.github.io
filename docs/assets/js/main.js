...
function toonToernooiGroepen(spelers, kalender = []) {
  ...
  const haalGroepOpties = () => ({
    gebruikGroepVier: gebruikGroepVierToggle ? gebruikGroepVierToggle.checked : true,
...
function verdeelSpelersInGroepen(spelers, opties = {}) {
  ...
  const actieveDefinities = gebruikGroepVier ? groepDefinities : groepDefinities.slice(0, 3);
  const groepen = actieveDefinities.map(definitie => ({ ...definitie, spelers: [] }));
